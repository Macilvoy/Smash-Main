...
local API_WEAPON = {"BoxingGloves","Sword"}
local API_ANIMATIONS ={"IDLE","AD","JUMP","JUMP2","FALLING","FALLINGRECOVER","HIT","HITRECOVER","HANGING","HANGINGRECOVER","DASH","SHIELD","SHIELDBROKE","GRABBING","GRABBER","GRABBED","GRABATTACK","J","Jx2","Jx3","Jx4","JW","JS","JAD","JSPACEW","JSPACES","JSPACEAD","K","KW","KS","KAD"}
local API_BONES = {"PelvisKeys","LHandKeys","RHandKeys","LLegKeys","RLegKeys"}
local API_CORDS = {"Position","Rotation"}
...
function OnPlayerJoined(player)
    ...
    player:SetResource(Ydir,0)
    player:SetResource(KeyActive,0)
    player:SetResource(CapsuleAttached,0)
    
    player:SetResource(WeaponId,1) -- TEMP RESOURCE
    --print(API[API_WEAPON[player:GetResource(WeaponId)]][API_ANIMATIONS[player:GetResource(StateId)]][API_BONES[1]][p1Keyframe][API_CORDS[1]])
...
function SpawnIKRig(player)
    if player:GetResource(PNum) == 1 then
        ...
        p1AnimFolder.equippedEvent:Connect(EquipingTrigger)
    else
...
function EquipingTrigger(equip, player)
...
if player:GetResource(PNum) == 1 then
    if player:GetResource(isAttached) == 1 then
        ...
        if player:GetWorldRotation().z > 0 then
        player:SetResource(Ydir,1)
        else
        player:SetResource(Ydir,-1)
        end
    --     print("p1Keyframe")
...
function ResyncAnimation(player)
    if player:GetResource(PNum) == 1 then
        ...
        if(p1Keyframe*KeyframeInterval >= p1Length) and player:GetResource(KeyActive) == 0 and p1AnimLength == 0 then
            if player:GetResource(Zpos) == 0 then
                player:SetResource(StateId,1)
                p1Keyframe = 1
            else
                ...
                p1Keyframe = 1
           elseif (p1Keyframe*KeyframeInterval >= p1Length) and player:GetResource(KeyActive) == 1 and p1AnimLength > 0 then
                p1Keyframe = 1
           elseif(p1Keyframe*KeyframeInterval >= p1Length) and player:GetResource(KeyActive) == 0 and p1AnimLength > 0 then
            p1Keyframe = 1
            end

        p1pelvisIK[1]=Vector3.New(API[API_WEAPON[player:GetResource(WeaponId)]][API_ANIMATIONS[player:GetResource(StateId)]][API_BONES[1]][p1Keyframe][API_CORDS[1]])
        p1pelvisIK[2]=Rotation.New(API[API_WEAPON[player:GetResource(WeaponId)]][API_ANIMATIONS[player:GetResource(StateId)]][API_BONES[1]][p1Keyframe][API_CORDS[2]])
        p1leftHIK[1]=Vector3.New(API[API_WEAPON[player:GetResource(WeaponId)]][API_ANIMATIONS[player:GetResource(StateId)]][API_BONES[2]][p1Keyframe][API_CORDS[1]])
        p1leftHIK[2]=Rotation.New(API[API_WEAPON[player:GetResource(WeaponId)]][API_ANIMATIONS[player:GetResource(StateId)]][API_BONES[2]][p1Keyframe][API_CORDS[2]])
        p1rightHIK[1]=Vector3.New(API[API_WEAPON[player:GetResource(WeaponId)]][API_ANIMATIONS[player:GetResource(StateId)]][API_BONES[3]][p1Keyframe][API_CORDS[1]])
        p1rightHIK[2]=Rotation.New(API[API_WEAPON[player:GetResource(WeaponId)]][API_ANIMATIONS[player:GetResource(StateId)]][API_BONES[3]][p1Keyframe][API_CORDS[2]])
        p1leftLIK[1]=Vector3.New(API[API_WEAPON[player:GetResource(WeaponId)]][API_ANIMATIONS[player:GetResource(StateId)]][API_BONES[4]][p1Keyframe][API_CORDS[1]])
        p1leftLIK[2]=Rotation.New(API[API_WEAPON[player:GetResource(WeaponId)]][API_ANIMATIONS[player:GetResource(StateId)]][API_BONES[4]][p1Keyframe][API_CORDS[2]])
        p1rightLIK[1]=Vector3.New(API[API_WEAPON[player:GetResource(WeaponId)]][API_ANIMATIONS[player:GetResource(StateId)]][API_BONES[5]][p1Keyframe][API_CORDS[1]])
        p1rightLIK[2]=Rotation.New(API[API_WEAPON[player:GetResource(WeaponId)]][API_ANIMATIONS[player:GetResource(StateId)]][API_BONES[5]][p1Keyframe][API_CORDS[2]])

        for num,val in pairs(API[API_WEAPON[player:GetResource(WeaponId)]][API_ANIMATIONS[player:GetResource(StateId)]][API_BONES[1]]) do
            if val==nil then
                break
            else
                p1Length=num*KeyframeInterval
            end
        end
        LocateIK(player)
        ...
        if p1AnimLength > 0 then
            p1AnimLength = p1AnimLength - 1
        end
...
function SetIK(anchor, player, IK)
    anchor:MoveTo(Vector3.New(IK[1]),KeyframeInterval/10,true)
    anchor:RotateTo(Rotation.New (IK[2]),KeyframeInterval/10,true)
